...
// -----------------------------------------------------------------------------
// Xenia Game Patches (.patch.toml) Support
// -----------------------------------------------------------------------------

template <typename T>
static void WriteGuestMemory(xe::Memory* memory, uint32_t address, T value) {
...
static void ApplyPatchFile(xe::Memory* memory, const std::filesystem::path& patch_path) {
  try {
    ...
    XELOGE("Unknown error applying patch TOML");
  }
}

void UserModule::ApplyGamePatches() {
  uint32_t tid = title_id();
  if (!tid) return;
  ...
  XELOGI("Searching for game patches with Title ID: {}", tid_str);

  for (const auto& dir : patch_dirs) {
    ...
    try {
      for (const auto& entry : std::filesystem::directory_iterator(dir)) {
        ...
        if (filename_upper.rfind(tid_str, 0) == 0 && filename_upper.find(".PATCH.TOML") != std::string::npos) {
          XELOGI("Found patch file: {}", entry.path().string());
          ApplyPatchFile(memory(), entry.path());
        }
      }
    } catch (const std::exception& ex) {
      XELOGE("Error searching directory {}: {}", dir.string(), ex.what());
    } catch (...) {
      XELOGE("Unknown error searching directory {}", dir.string());
    }
  }
}
...
X_STATUS UserModule::LoadXexContinue() {
  ...
  // Apply Canary game patches (.patch.toml) immediately after load is done, before CPU starts executing
  ApplyGamePatches();
