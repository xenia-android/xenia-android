...
X_STATUS Emulator::LaunchPath(const std::filesystem::path& path) {
  ...
  if (file.is_open()) {
    ...
    if (file.read(bytes, 4)) {
      magic = (static_cast<uint32_t>(static_cast<uint8_t>(bytes[0])) << 24) |
  ...
  // Choose the launch method based on magic bytes first
  if (magic == 0x58455832 || magic == 0x58455831 || magic == 0x7F454C46) {
    // "XEX2", "XEX1" or "\x7FELF"
    return LaunchXexFile(path);
  } else if (magic == 0x434F4E20 || magic == 0x50495253 || magic == 0x4C495645) {
    // "CON ", "PIRS" or "LIVE"
